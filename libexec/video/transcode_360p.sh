...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --description)
      ...
      ;;
    *)
      echo "Invalid argument for transcode_360p.sh: $1" >&2
      exit 1
      ;;
  esac
  shift
...
if [ $FFMPEG_EXIT_CODE -ne 0 ]; then
    echo "❌ 360p encoding failed (exit code: $FFMPEG_EXIT_CODE)"
    exit 1
fi

echo "✅ 360p encoding complete"
